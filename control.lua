...
script.on_event({
    ...
    function(event)
        if LauncherStation.is_station_entity(event.entity.name) then
            local settings = LauncherStation.read_settings(event.tags)
            LauncherStation.create(event.entity, settings)
        end
        if ReceiverStation.is_station_entity(event.entity.name) then
...
script.on_event(defines.events.on_entity_settings_pasted, function(event)
    ...
    if LauncherStation.is_station_entity(event.source.name) then
        local source = LauncherStation.get(event.source)
        if source then
            launcher_settings = source.settings
        end
    end
    if event.source.name == "entity-ghost" and LauncherStation.is_station_entity(event.source.ghost_name) then
        launcher_settings = LauncherStation.read_settings(event.source.tags)
    end
    if launcher_settings then
        if LauncherStation.is_station_entity(event.destination.name) then
            local destination = LauncherStation.get(event.destination)
            if destination then
                destination:set_settings(launcher_settings)
            end
        end
        if event.destination.name == "entity-ghost" and LauncherStation.is_station_entity(event.destination.ghost_name) then
    ...
    if ReceiverStation.is_station_entity(event.source.name) then
        local source = ReceiverStation.get(event.source)
        if source then
            receiver_settings = source.settings
        end
    end
    if event.source.name == "entity-ghost" and ReceiverStation.is_station_entity(event.source.ghost_name) then
        receiver_settings = ReceiverStation.read_settings(event.source.tags)
    end
    if receiver_settings then
        if ReceiverStation.is_station_entity(event.destination.name) then
            local destination = ReceiverStation.get(event.destination)
            if destination then
                destination:set_settings(receiver_settings)
            end
        end
        if event.destination.name == "entity-ghost" and ReceiverStation.is_station_entity(event.destination.ghost_name) then
            local tags = event.destination.tags or {}
...
script.on_event(defines.events.on_post_entity_died, function(event)
    ...
    -- Launcher
    if LauncherStation.is_station_entity(event.ghost.ghost_name) then
    ...
    -- Receiver
    if ReceiverStation.is_station_entity(event.ghost.ghost_name) then
...
script.on_event(defines.events.on_player_setup_blueprint, function(event)
    ...
    for index, entity in ipairs(entities) do
        -- Launcher
        if LauncherStation.is_station_entity(entity.name) then
        ...
        -- Receiver
        if ReceiverStation.is_station_entity(entity.name) then
            local source = event.mapping.get()[index] --[[@as LuaEntity?]]
            if source then
                local receiver_settings = nil
                if source.name == "entity-ghost" then
...
script.on_event({ constants.rotate_input_event, constants.reverse_rotate_input_event }, function(event)
    ...
    if player.selected and LauncherStation.is_station_entity(player.selected.name) then
...
local function update_player_selected_diode_status(player)
    if player.selected then
        if LauncherStation.is_station_entity(player.selected.name) then
            local launcher = LauncherStation.get(player.selected)
            if launcher then launcher:update_diode_status() end
        elseif ReceiverStation.is_station_entity(player.selected.name) then
...
script.on_event(defines.events.on_tick, function(event)
    ...
    for _, player in ipairs(game.connected_players) do
        ...
        if tick_index == 0 and player.opened and player.opened.object_name == "LuaEntity" then
            if LauncherStation.is_gui_entity(player.opened.name) then
            ...
            if ReceiverStation.is_gui_entity(player.opened.name) then
                local entity = player.opened --[[@as LuaEntity]]
...
script.on_event({
    ...
    elseif LauncherStation.is_station_entity(entity.name) then
        local launcher = LauncherStation.get(entity)
