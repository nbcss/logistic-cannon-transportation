...
local health = 800
local range = settings.startup[constants.setting_launcher_base_range].value --[[@as integer]]
local energy_consumption = settings.startup[constants.setting_launcher_base_consumption].value --[[@as integer]]
...
local launcher_animation = function(frame_count, run_mode)
    ...
    if frame_count and frame_count < 0 then
        launcher.x = launcher.width * (4 + frame_count)
        shadow.x = shadow.width * (4 + frame_count)
    end
    return { launcher, shadow }
end
...
data:extend {
    ...
    },
    {
        ...
        minable = { mining_time = 0.5, result = constants.item_launcher },
